...
export function quantizeNoteSequence(
  ...
  if (qns.timeSignatures.length > 0) {
    ...
    for (let i = 1; i < qns.timeSignatures.length; i++) {
      ...
      if (
        timeSignature.numerator !== qns.timeSignatures[0].numerator ||
...
export function assertIsAbsoluteQuantizedSequence(ns: INoteSequence) {
  if (!isAbsoluteQuantizedSequence(ns)) {
    throw new QuantizationStatusException(
      `NoteSequence ${ns.id} is not quantized or is quantized based on relative timing`
    );
  }
}
...
export function mergeInstruments(ns: INoteSequence) {
  ...
  const events = result.notes
    .concat(result.pitchBends)
    .concat(result.controlChanges);
  const programs = Array.from(
    new Set(events.filter((e) => !e.isDrum).map((e) => e.program))
  );
...
export function replaceInstruments(
  ...
  const instrumentsInOriginal = new Set(
    originalSequence.notes.map((n) => n.instrument)
  );
  const instrumentsInReplace = new Set(
    replaceSequence.notes.map((n) => n.instrument)
  );
 ...
 durations does not match.`);
  ...
  if (isQuantizedSequence(concatenateSequences[0])) {
    ...
    for (let i = 0; i < concatenateSequences.length; ++i) {
      ...
      if (
        concatenateSequences[i].quantizationInfo.stepsPerQuarter !==
...
function concatenateHelper(
  ...
  for (let i = 0; i < seqs.length; ++i) {
    const seqDuration = sequenceDurations
      ? sequenceDurations[i]
      : seqs[i][totalKey];
    if (seqDuration === 0) {
      throw Error(
        `Sequence ${seqs[i].id} has no ${totalKey}, and no individual duration was provided.`
      );
    }
...
function trimHelper(
  ...
  for (let i = 0; i < result.notes.length; i++) {
    ...
    if (truncateEndNotes) {
      result.notes[i][endKey] = Math.min(
        result.notes[i][endKey],
        result[totalKey]
      );
    }
  }
...
export function split(seq: INoteSequence, chunkSize: number): NoteSequence[] {
  ...
  // Sort notes first.
  const notesBystartStep = ns.notes.sort(
  ...
  for (let i = 0; i < notesBystartStep.length; i++) {
    ...
    if (note.quantizedEndStep <= chunkSize) {
      ...
      if (
        note.quantizedEndStep > chunkSize ||
